...
function Dashboard() {
  ...
  const { data: stats, isLoading: statsLoading, error: statsError } = useQuery({ queryKey: ['dashboard-stats'], queryFn: fetchDashboardStats });
  const { data: assets, isLoading: assetsLoading, error: assetsError } = useQuery({ queryKey: ['assets'], queryFn: fetchAssets });
  const { data: users, isLoading: usersLoading, error: usersError } = useQuery({ queryKey: ['users'], queryFn: fetchUsers });
  const { data: recentActivity, isLoading: historyLoading, error: historyError } = useQuery({ queryKey: ['history'], queryFn: fetchHistory });

  if (statsLoading || assetsLoading || usersLoading || historyLoading) {
    return <div className="p-6">Loading dashboard data...</div>;
  }

  const dashboardError = statsError || assetsError || usersError || historyError;
  if (dashboardError) {
    return (
      <div className="p-6 max-w-[1600px] mx-auto">
        <div className="border border-destructive/30 bg-destructive/10 text-destructive rounded-lg p-4">
          <h1 className="font-semibold">Dashboard data could not load.</h1>
          <p className="text-sm mt-1">
            Check that the backend API and MySQL connection are working, then refresh this page.
          </p>
        </div>
      </div>
    );
